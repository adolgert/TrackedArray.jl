...
const DealerStateRef = Ref{Any}(nothing)
...
Base.getindex(v::DealerVector{T}, i::Integer) where T = begin
    element = v.data[i]
...
Base.setindex!(v::DealerVector{T}, x, i::Integer) where T = begin
    v.data[i] = x
...
function notify_read(v::DealerVector, index::Int, field::Symbol)
    if v.physical_state !== nothing
        notify_read(v.physical_state, v.array_name, index, field)
    end
end

function notify_write(v::DealerVector, index::Int, field::Symbol)
    if v.physical_state !== nothing
...
gotten(v::DealerVector) = Set{Tuple}()  # Tracking is at state level
changed(v::DealerVector) = Set{Tuple}()  # Tracking is at state level
reset_tracking!(v::DealerVector) = v
reset_gotten!(v::DealerVector) = v
...
function changed(state::DealerState)
    return getfield(state, :_writes)
end

function wasread(state::DealerState)
    return getfield(state, :_reads)
end

function accept(state::DealerState)
    empty!(getfield(state, :_reads))
    empty!(getfield(state, :_writes))
    return state
end

function resetread(state::DealerState)
    empty!(getfield(state, :_reads))
    return state
end
...
Creates a DealerState with DealerVector arrays populated with observable structs.
"""
function ConstructState(specification, counts)
    # Generate element types and create DealerVectors
    fields = []
    ...
    for (array_name, field_specs) in specification
        ...
        # Create DealerVector
        dealer_vec = DealerVector{element_type}(undef, count)
        dealer_vec.array_name = array_name
        
        push!(fields, array_name => dealer_vec)
    end
